...
parser = argparse.ArgumentParser(
    description="""Variant identity file generator: Creating VIF file for DiMSum pipeline from Nanopore polishing output"""
)
...
parser.add_argument(
    "--probe_UMI_down",
    help="A short sequence (eg 14 bp) beginning 4 bp downstream of the UMI in fasta format.",
    required=True,
)
parser.add_argument(
    "--probe_UMI_up", help="A short sequence (eg 14 bp) ending 4 bp upstream of the UMI in fasta format.", required=True
)
parser.add_argument(
    "--probe_frame_down",
    help="A short sequence (eg 10 bp) flanking the frame (downstream) in fasta format.",
    required=True,
)
...
for c in clusters:
    ...
    if (
        UMI == "UMI_not_detectable"
    ):  # if flexible solve (with fixed difference between up and down was not succesful, cluster is counted as not)
        no_UMI_count += 1
        print("patternmatch (UMI) not succesful")
        continue  # if UMI not detectable it is not written in the .txt file

    if (
        frame == "frame_not_detectable"
    ):  # if flexible solve (with fixed difference between up and down was not succesful, cluster is counted as not)
        no_frame_count += 1
